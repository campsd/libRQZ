...
contains
  ...
  ! last edit: January 11, 2019
    ...
    do while ((nd + nnd) .lt. wi-1)
      ...
      if (ntm) then ! Need to move to back
        j = cstrt + nd + 1 ! Index in large matrix
        i = nd + 1  ! Index in small matrix
        if (abs(A(j+1,j)) .gt. dzero) then
          ! This is a 2x2 block
          do while (i .lt. wi - nnd)
            if (abs(A(j+3,j+2)) .gt. dzero) then
              call d_swap22(j, j, A, B, .true., apCI, apTI, QW, .true., i, &
                     ZW, .true., i)
              j = j + 2
              i = i + 2
            else
              call d_swap21(j, j, A, B, .true., apCI, apTI, QW, .true., i, &
                     ZW, .true., i)
              j = j + 1
              i = i + 1
            end if
          ...
          do while (i .lt. wi - nnd)
            if (abs(A(j+2,j+1)) .gt. dzero) then
              call d_swap12(j, j, A, B, .true., apCI, apTI, QW, .true., i, &
                     ZW, .true., i)
              j = j + 2
              i = i + 2
            else
              call d_swap11(j, j, A, B, .true., apCI, apTI, QW, .true., i, &
                     ZW, .true., i)
              j = j + 1
              i = i + 1
            end if
  ...
  ! last edit: January 11, 2019
    ...
    do while ((nd + nnd) .lt. wi-1)
      ...
      if (ntm) then ! Need to move
        j = cstp-nd ! Index in large matrix
        i = wi-nd ! Index in small matrix
